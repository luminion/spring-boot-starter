 ...
 * <p>
 * Jackson3的主要变化：
 * 1. 使用JsonMapper替代部分ObjectMapper的使用（实际上JsonMapper继承自ObjectMapper）
 * 2. 更好的默认配置和安全性改进
 * 3. API向后兼容，大部分API与Jackson2相同
 *
 * @author luminion
...
@Slf4j
@AutoConfiguration
@ConditionalOnClass({ObjectMapper.class, JsonMapper.class})
...
public class Jackson3AutoConfiguration {
    ...
    static class Jackson2ObjectMapperBuilderCustomizerConfiguration {
        ...
        public Jackson2ObjectMapperBuilderCustomizer jackson2ObjectMapperBuilderCustomizer(DateTimeFormatProperties dateTimeFormatProperties) {
            log.debug("Jackson2ObjectMapperBuilderCustomizer Configured (Jackson3)");
            return builder -> {
    ...
    static class JacksonRedisConfiguration {
        ...
        public RedisSerializer<Object> redisSerializer(Jackson2ObjectMapperBuilder jackson2ObjectMapperBuilder) {
            // Jackson3中，ObjectMapper和JsonMapper的API基本一致，可以直接使用
            // 但为了更好的兼容性，我们仍然使用build()方法获取ObjectMapper
            ...
            // 注意：Jackson3中activateDefaultTyping方法已被标记为deprecated，但为了兼容性仍然使用
            // TODO: 考虑迁移到新的PolymorphicTypeValidator API
