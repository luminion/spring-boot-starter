...
class VeloJacksonAutoConfigurationTests {
    ...
    void shouldSerializeBigIntegerAsStringUnconditionallyEvenForSmallValues() throws Exception {
        contextRunner
                ...
                .run(context -> {
                    ...
                    JsonNode small = mapper.readTree(mapper.writeValueAsString(
                    ...
                    JsonNode large = mapper.readTree(mapper.writeValueAsString(
    ...
    void shouldSerializeScientificBigDecimalAsPlainTextByDefault() throws Exception {
        contextRunner
                ...
                .run(context -> {
                    JsonMapper mapper = jsonMapper(context);
                    JsonNode tree = mapper.readTree(mapper.writeValueAsString(new ScientificAmountPayload(
                            new BigDecimal("1E+3"))));
    ...
    void shouldIgnoreInvalidDateTimePatternsWhenJacksonDateTimeCustomizationDisabled() throws Exception {
        ...
        contextRunner
                ...
                .run(context -> {
                    ...
                    assertThat(json).contains("\"createdAt\":\"2026-03-31T08:09:10\"");
                    assertThat(json).doesNotContain("\"createdAt\":\"2026-03-31 08:09:10\"");
                });
    }
    ...
    void shouldUseFlexibleDefaultDateFormatAndHonorJsonFormat() throws Exception {
        contextRunner
                ...
                .run(context -> {
                    JsonMapper mapper = jsonMapper(context);
                    DatePayload payload = mapper.readValue(
    ...
    void shouldKeepUnannotatedStringsUnchangedWhenJacksonXssTargetIsDisabled() {
        ...
        contextRunner
                ...
                .run(context -> {
                    JsonMapper mapper = jsonMapper(context);
                    StringTransformPayload payload = mapper.readValue(
