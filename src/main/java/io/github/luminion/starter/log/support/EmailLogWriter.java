...
public class EmailLogWriter extends RequestLogWriter {
    ...
    public void error(Object target, MethodSignature signature, Object[] args,
     ...
     */
    private void sendErrorEmail(MethodSignature signature, Object[] args,
                                Throwable throwable, long duration) {
        try {
...
// InvokeLog the exception that occurred while sending email
            System.err.println("Error sending email notification: " + e.getMessage());
        }
    }

    private String buildHtmlEmailBody(String appName, String port, String
            requestInfo, String methodName, String methodArgs, long duration, String
                                              stackTrace) {
        return "<!DOCTYPE html>" +
                "<html>" +
                "<head>" +
                ...
                "h2 {color: #D32F2F; border-bottom: 2px solid #f2f2f2; padding-bottom: 10 px;}" +
                ...
                "pre {white-space: pre-wrap; word-wrap: break-word; background-color:#f5f5f5;padding:15px;border-radius:4px;border:1 px solid #ccc; font-family:'Courier New',Courier,monospace;}" +
                "</style>" +
                "</head>" +
                "<body>" +
