...
import io.github.luminion.starter.feature.jackson.annotation.JsonDecrypt;
...
/**
 * {@link JsonDecrypt} 注解格式化工厂
 *
 * @author luminion
 */
@RequiredArgsConstructor
public class UnmaskAnnotationFormatterFactory implements AnnotationFormatterFactory<JsonDecrypt> {
    ...
    @Override
    public Printer<?> getPrinter(JsonDecrypt annotation, Class<?> fieldType) {
        return (obj, locale) -> (String) obj;
    }

    @Override
    public Parser<?> getParser(JsonDecrypt annotation, Class<?> fieldType) {
        // 1. 从容器中获取 @JsonDecrypt 指定的解密/还原策略 Bean
