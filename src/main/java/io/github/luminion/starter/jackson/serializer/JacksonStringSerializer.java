...
import io.github.luminion.starter.core.annotation.StringEncode;
...
/**
 * 统一字符串序列化处理器
 * 仅处理 String 字段。用于处理 @StringEncode 注解（如脱敏）
 *
 * @author luminion
 */
@Slf4j
public class JacksonStringSerializer extends StdSerializer<String> implements ContextualSerializer {
    ...
    public JsonSerializer<?> createContextual(SerializerProvider prov, BeanProperty property)
        ...
        StringEncode encodeAnn = property.getAnnotation(StringEncode.class);
        if (encodeAnn != null) {
            ...
            try {
                ...
                String errorMsg = String.format("未发现 @StringEncode 指定的函数类 [%s] 的 Bean 实例。", funcClass.getName());
                log.error(errorMsg);
